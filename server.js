...
app.post('/api/admin/exhibitions', authenticateToken, authorizeRoles('admin', 'editor'), upload.single('image'), async (req, res) => {
  try {
    const data = { ...req.body };
    if (req.file) {
...
// Create Event
app.post('/api/admin/events', authenticateToken, authorizeRoles('admin', 'editor'), async (req, res) => {
  try {
